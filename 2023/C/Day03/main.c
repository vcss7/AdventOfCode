...
bool is_next_to_symbol(char* prev_line, char* curr_line, char* next_line, int i);
    ...
    while(!feof(fptr))
    {
        ...
        for(int i = 0; i < strlen(curr_line); i++)
        {
            if(is_digit(curr_line[i]))
            {
                ...
                if(i < strlen(curr_line) && !is_digit(curr_line[i + 1]) && num_is_next_to_symbol)
                {
                    sum += num;
                }
...
bool is_next_to_symbol(char* prev_line, char* curr_line, char* next_line, int i)
{
    if(i > 0 && i < strlen(curr_line) - 1)
    {
        ...
        {
            if(is_symbol(prev_line[j]) || is_symbol(curr_line[j]) || is_symbol(next_line[j]))
                return true;
        }
    }
    return false;
