...
int main(void)
{
    ...
    while(fgets(line, sizeof(line), fptr))
    {
        ...
        for (int i = 0; i < num_count; i++)
        {
            ...
            if (a > b) mon = MONOTONICITY_DECREASING;
            else if (a < b) mon = MONOTONICITY_INCREASING;
            ...
            for (int j = 0; j < num_count - 2; j++)
            {
                ...
                switch (mon)
                {
                    case MONOTONICITY_DECREASING:
                        safe_step = diff > 0;
                        break;
                    case MONOTONICITY_INCREASING:
                        safe_step = diff < 0;
                        break;
                }
